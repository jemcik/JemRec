...
The workflow derives `versionName` from the tag and `versionCode` from
`MAJOR*10000 + MINOR*100 + PATCH`, builds a release APK signed with the project
key held in repository secrets, refuses to publish anything `apksigner` cannot
verify, and attaches `jemrec-0.2.apk` to the release.
